...
# Define pi constant as a torch tensor 
...
class Blur(nn.Module):
    ...
    # Specific internal forward method placeholder
...
class DecomposeLattice(Lattice):
    ...
    def _pca(self, input_lattice):
        # TODO: Check if it's decomposed already.
        ...
        # allocate coefs
        # TODO: Check if shape[2] is really 'self.rank+1'
...
class PnPADMMDeblur(Deblur):
    ...
    def _forward(self, y, x_true=None, debug=True):
        ...
        lamb = self.deblur_pars.get('lamb', torch.tensor(0.1))
        gamma = self.deblur_pars.get('gamma', torch.tensor(1.0))
        x = self.deblur_pars.get('x0', torch.zeros(y.shape).to(device))
